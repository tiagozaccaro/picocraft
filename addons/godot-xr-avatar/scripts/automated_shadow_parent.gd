...
@export var max_body_angle := 30.0 # (float, 0.0, 120.0, 5.0)
@export var body_turn_duration := 1.0 # (float, 0.1, 5.0)
...
@onready var arvrcamera = $FPController/XRCamera3D
...
@onready var skeleton : Skeleton3D = $FPController/avatar/Armature/Skeleton3D
...
func _ready():
	...
	if substitute_head_bone == false:
		avatar_height = $FPController/avatar/Armature/Skeleton3D/character_height.transform.origin.y
		...
		if get_node_or_null("FPController/avatar/Armature/Skeleton3D/HeadEndBone") != null:
			avatar_height = get_node("FPController/avatar/Armature/Skeleton3D/HeadEndBone").transform.origin.y
		#if there wasn't a head end bone and not a spatial substitute, use rough math for where the head end bone likely would be
		else:
			avatar_height = $FPController/avatar/Armature/Skeleton3D/character_height.transform.origin.y + .25
	...
	left_target_transform.rotation_degrees.x = left_foot.rotation_degrees.x + $FPController/avatar/Armature/Skeleton3D.rotation_degrees.x
	left_target_transform.rotation_degrees.y = left_foot.rotation_degrees.y + $FPController/avatar/Armature/Skeleton3D.rotation_degrees.y
	left_target_transform.rotation_degrees.z = left_foot.rotation_degrees.z + $FPController/avatar/Armature/Skeleton3D.rotation_degrees.z
	...
	right_target.add_child(right_target_transform, true)
		#match target rotations to bone attachment rotations which are avatar-specific
	right_target_transform.rotation_degrees.x = right_foot.rotation_degrees.x + $FPController/avatar/Armature/Skeleton3D.rotation_degrees.x
	right_target_transform.rotation_degrees.y = right_foot.rotation_degrees.y + $FPController/avatar/Armature/Skeleton3D.rotation_degrees.y
	right_target_transform.rotation_degrees.z = right_foot.rotation_degrees.z + + $FPController/avatar/Armature/Skeleton3D.rotation_degrees.z
	...
	$FPController/XRCamera3D.transform = get_arvr_camera_transform()
	...
	if auto_anim_choice == AutomaticAnimation.NO:
		...
		animationplayer.name = "AnimationPlayer"
		$FPController/avatar.add_child(animationplayer,true)
		animationtree = load("addons/godot-xr-avatar/animations/mixamo_animations/AnimationTree-mixamo-complete.tscn").instantiate()
		animationtree.name = "AnimationTree"
		$FPController/avatar.add_child(animationtree, true)
		animationtree.active = true
...
func process_procedural_walk(delta: float, move: Vector2) -> void:
	...
	if (abs(move.y) <= .1 and abs(move.x) <= .1) or player_player_body.on_ground == false:
	...
	if is_walking_legs:
		...
		# set where we want the legs to go, set just a bit in the direction ahead of where the player is going (because shadow is mirrored), opposite of normal avatar
		desired_l_leg_pos = left_target.global_transform.origin + -player_player_body.velocity.normalized()*step_distance
		desired_r_leg_pos = right_target.global_transform.origin + -player_player_body.velocity.normalized()*step_distance
...
func _physics_process(delta):
	...
	var avatar_pos: Vector3 = arvrorigin.global_transform * (Plane.PLANE_XZ.project(arvrcamera.transform.origin))
	var avatar_dir_z: Vector3 = arvrorigin.global_transform.basis * (body_direction).normalized()
	var avatar_dir_x := Vector3.UP.cross(avatar_dir_z)
	$FPController/avatar/Armature.global_transform = Transform3D(avatar_dir_x, Vector3.UP, avatar_dir_z, avatar_pos)
