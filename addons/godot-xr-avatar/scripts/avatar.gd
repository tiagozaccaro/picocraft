...
@export var head_mesh_node_paths = [] # (Array, NodePath)
...
@onready var left_hand : Node3D = get_node(left_hand_path)
@onready var right_hand : Node3D = get_node(right_hand_path)
@onready var player_body := PlayerBody.get_player_body(self)
@onready var left_foot : BoneAttachment3D = $Armature/Skeleton3D/left_foot
@onready var right_foot : BoneAttachment3D = $Armature/Skeleton3D/right_foot
@onready var LL_ik : SkeletonIK3D = $Armature/Skeleton3D/SkeletonIKLegL
@onready var RL_ik : SkeletonIK3D = $Armature/Skeleton3D/SkeletonIKLegR
@onready var skeleton : Skeleton3D = $Armature/Skeleton3D

func _ready():
	#set avatar height to player
	avatar_height = $Armature/Skeleton3D/character_height.transform.origin.y
	$Armature.scale *= get_current_player_height()/$Armature/Skeleton3D/character_height.transform.origin.y
...
#	print($Armature.scale)
	...
	if left_controller_path != null:
		$Armature/Skeleton3D/SkeletonIKL.set_target_node(NodePath("../../../../" + left_controller.name + "/" + left_hand.name + "/left_target"))
	else:
		print("Left controller path not found, assuming just using hand node.")
		$Armature/Skeleton3D/SkeletonIKL.set_target_node(NodePath("../../../../" + left_hand.name + "/left_target"))
	
	if right_controller_path != null:
		$Armature/Skeleton3D/SkeletonIKR.set_target_node(NodePath("../../../../" + right_controller.name + "/" + right_hand.name + "/right_target"))
	else:
		print("Right controller path not found, assuming just using hand node.")
		$Armature/Skeleton3D/SkeletonIKR.set_target_node(NodePath("../../../../" + right_hand.name + "/right_target"))
	...
	#start the IK
	$Armature/Skeleton3D/SkeletonIKL.start()
	$Armature/Skeleton3D/SkeletonIKR.start()
	LL_ik.start()
	RL_ik.start()
	...
	if head_visible == false:
		for mesh_path in head_mesh_node_paths:
			var head_mesh_part : MeshInstance3D = get_node(mesh_path)
