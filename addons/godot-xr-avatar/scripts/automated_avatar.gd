...
@export var left_hand : XRToolsHand = null
@export var right_hand : XRToolsHand = null
...
@export var face_mesh_with_visemes : MeshInstance3D = null
...
@export var auto_anim_choice: AutomaticAnimation = AutomaticAnimation.NO
...
@export var max_body_angle : float = 30.0 # (float, 0.0, 120.0, 5.0)
@export var body_turn_duration : float = 1.0 # (float, 0.1, 5.0)


#variables used for SkeletonIK nodes and bone attachment nodes
var left_foot : BoneAttachment3D = null
var right_foot : BoneAttachment3D = null
var character_height : BoneAttachment3D = null
var SkeletonIKL : SkeletonIK3D = null
var SkeletonIKR : SkeletonIK3D = null
var SkeletonIKLegL : SkeletonIK3D = null
var SkeletonIKLegR : SkeletonIK3D = null
...
var left_target : Marker3D = null
var right_target : Marker3D = null
var left_target_transform : Marker3D = null
var right_target_transform : Marker3D = null
var Raycast_L : RayCast3D = null
var Raycast_R : RayCast3D = null
...
#set all nodes 
@onready var player_body = xr_origin.get_node("PlayerBody")
...
func _ready():
	...
	skeleton.position.z = avatar_z_offset
	skeleton.position.x = avatar_x_offset
	...
	character_height = BoneAttachment3D.new()
	...
	SkeletonIKLegL = SkeletonIK3D.new()
	...
	SkeletonIKLegR = SkeletonIK3D.new()
	...
	if substitute_head_bone == false:
		...
		if get_node_or_null("Armature/Skeleton3D/HeadEndBone") != null:
			avatar_height = get_node("Armature/Skeleton3D/HeadEndBone").transform.origin.y
	...
	left_target_transform.rotation_degrees.x = left_foot.rotation_degrees.x + $Armature/Skeleton3D.rotation_degrees.x
	left_target_transform.rotation_degrees.y = left_foot.rotation_degrees.y + $Armature/Skeleton3D.rotation_degrees.y
	left_target_transform.rotation_degrees.z = left_foot.rotation_degrees.z + $Armature/Skeleton3D.rotation_degrees.z
	
	right_target = Marker3D.new()
	right_target.name = "RL_c"
	add_child(right_target, true)
	
		#used when hooking to right leg SkeletonIK
	right_target_transform = Marker3D.new()
	...
	right_target_transform.rotation_degrees.x = right_foot.rotation_degrees.x + $Armature/Skeleton3D.rotation_degrees.x
	right_target_transform.rotation_degrees.y = right_foot.rotation_degrees.y + $Armature/Skeleton3D.rotation_degrees.y
	right_target_transform.rotation_degrees.z = right_foot.rotation_degrees.z + + $Armature/Skeleton3D.rotation_degrees.z
	...
	if use_automated_lipsync == true:
		if face_mesh_with_visemes == null:
	...
	if auto_anim_choice == AutomaticAnimation.NO:
		...
		animationplayer = load("res://addons/godot-xr-avatar/animations/make_human_animations/AnimationPlayer.tscn").instantiate()
		animationplayer.name = "AnimationPlayer"
		add_child(animationplayer,true)
		animationtree = load("res://addons/godot-xr-avatar/animations/make_human_animations/AnimationTree-MH-Complete.tscn").instantiate()
		animationtree.name = "AnimationTree"
		add_child(animationtree, true)
		animationtree.active = true
	
	elif auto_anim_choice == AutomaticAnimation.MIXAMO:
		animationplayer = load("res://addons/godot-xr-avatar/animations/mixamo_animations/AnimationPlayer.tscn").instantiate()
		animationplayer.name = "AnimationPlayer"
		add_child(animationplayer,true)
		animationtree = load("res://addons/godot-xr-avatar/animations/mixamo_animations/AnimationTree-mixamo-complete.tscn").instantiate()
		animationtree.name = "AnimationTree"
		add_child(animationtree, true)
		animationtree.active = true
		
	elif auto_anim_choice == AutomaticAnimation.READYPLAYERME:
		animationplayer = load("res://addons/godot-xr-avatar/animations/ready_player_animations/AnimationPlayer.tscn").instantiate()
		animationplayer.name = "AnimationPlayer"
		add_child(animationplayer,true)
		animationtree = load("res://addons/godot-xr-avatar/animations/ready_player_animations/AnimationTree-Readyplayer-Complete.tscn").instantiate()
		animationtree.name = "AnimationTree"
		add_child(animationtree, true)
		animationtree.active = true	
	...
	body_direction = Plane.PLANE_XZ.project(xr_camera.transform.basis.z).normalized()


#function use to place avatar feet on surfaces procedurally
func update_ik_anim(target: Node3D, raycast: RayCast3D, bone_attach: BoneAttachment3D, d_b: Basis, avatar_height: float, hit_offset: float) -> void:
...
#	face_mesh.set("blend_shapes/viseme_sil", Viseme_Sil)
	...
	if blinktime >= blink_at:
		...
		await get_tree().create_timer(.33).timeout  # average blink time is 1/3 of a second
...
func _physics_process(delta: float) -> void:
	...
	var head_direction := Plane.PLANE_XZ.project(xr_camera.transform.basis.z).normalized()
	var body_angle := rad_to_deg(head_direction.signed_angle_to(body_direction, Vector3.UP))

	# Clamp the body angle and step it towards zero
	body_angle = clamp(body_angle, -max_body_angle, max_body_angle)
	var turn_scale : float = clamp(abs(body_angle) / 10.0, 0.2, 1.0)
	...
	body_direction = head_direction.rotated(Vector3.UP, deg_to_rad(body_angle)).normalized()

	# Move the avatar under the camera and facing in the direction of the body
	var avatar_pos: Vector3 = xr_origin.global_transform * (Plane.PLANE_XZ.project(xr_camera.transform.origin))
	var avatar_dir_z: Vector3 = xr_origin.global_transform.basis * (body_direction).normalized()
	var avatar_dir_x := Vector3.UP.cross(avatar_dir_z)
	$Armature.global_transform = Transform3D(avatar_dir_x, Vector3.UP, avatar_dir_z, avatar_pos)
...
#	var angles := xrcamera.rotation
#	angles.x *= -1; angles.z *= -1
#	head.basis = Basis(angles)
	head.basis.x = Vector3(xr_camera.global_transform.basis.x.x, -xr_camera.global_transform.basis.x.y, xr_camera.global_transform.basis.x.z)
	head.basis.y = Vector3(-xr_camera.global_transform.basis.y.x, xr_camera.global_transform.basis.y.y, xr_camera.global_transform.basis.y.z)
	head.basis.z = Vector3(xr_camera.global_transform.basis.z.x, -xr_camera.global_transform.basis.z.y, xr_camera.global_transform.basis.z.z)
	...
	if left_controller != null and right_controller != null and get_node_or_null("AnimationTree") != null:
		$AnimationTree.set("parameters/lefthandpose/blend_amount", left_controller.get_float("Grip"))
		$AnimationTree.set("parameters/righthandpose/blend_amount", right_controller.get_float("Grip"))
		$AnimationTree.set("parameters/lefthandposetrig/blend_amount", left_controller.get_float("Trigger"))
		$AnimationTree.set("parameters/righthandposetrig/blend_amount", right_controller.get_float("Trigger"))
	...
	if abs(move.y) > .25 and get_current_player_height() < (.9 * max_height):
		if turn_character_180 == true:
			$Armature/Skeleton3D/SkeletonIKLegL.magnet = lerp($Armature/Skeleton3D/SkeletonIKLegL.magnet, Vector3(1,3,3), delta)
			$Armature/Skeleton3D/SkeletonIKLegR.magnet = lerp($Armature/Skeleton3D/SkeletonIKLegR.magnet, Vector3(-1,3,3), delta)
		else:
			$Armature/Skeleton3D/SkeletonIKLegL.magnet = lerp($Armature/Skeleton3D/SkeletonIKLegL.magnet, Vector3(-1,3,3), delta)
			$Armature/Skeleton3D/SkeletonIKLegR.magnet = lerp($Armature/Skeleton3D/SkeletonIKLegR.magnet, Vector3(1,3,3), delta)
	else:
		if turn_character_180 == true:
			$Armature/Skeleton3D/SkeletonIKLegL.magnet = lerp($Armature/Skeleton3D/SkeletonIKLegL.magnet, Vector3(.2,0,1), delta)
			$Armature/Skeleton3D/SkeletonIKLegR.magnet = lerp($Armature/Skeleton3D/SkeletonIKLegR.magnet, Vector3(-.2,0,1), delta)
		else:
			$Armature/Skeleton3D/SkeletonIKLegL.magnet = lerp($Armature/Skeleton3D/SkeletonIKLegL.magnet, Vector3(-.2,0,1), delta)
			$Armature/Skeleton3D/SkeletonIKLegR.magnet = lerp($Armature/Skeleton3D/SkeletonIKLegR.magnet, Vector3(.2,0,1), delta)
...
func get_current_player_height() -> float:
	return xr_camera.transform.origin.y
